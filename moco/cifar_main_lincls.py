...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for name, param in model.named_parameters():
        if args.arch in ["resnet50", "resnet18", "resnet34"] and (name not in ['fc.weight', 'fc.bias']):
            param.requires_grad = False
        if args.arch in ['efficientb0', 'efficientb1'] and (name not in ['classifier.weight','classifier.bias']):
            param.requires_grad = False
...
#            for k in list(state_dict.keys()):
#                k_new = k[7:]
#                if args.arch in ['efficientb0']  and not k_new.startswith('classifier'):
#                    state_dict[k_new] = state_dict[k]
#                if args.arch in ['resnet18']  and not k_new.startswith('fc'):
#                    state_dict[k_new] = state_dict[k]
#                del state_dict[k]
...
#            print (msg)
    ...
    # init the fc layer
    if args.arch in ["efficientb0", "efficientb1"]:
