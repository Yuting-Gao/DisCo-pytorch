...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.arch == 'mobilenetv3':
        ...
        model = mobilenetv3.mobilenetv3_large_100(num_classes=1000)
    ...
    for name, param in model.named_parameters():
        if args.arch not in ["efficientb0", "mobilenetv3"] and (name not in ['fc.weight', 'fc.bias']):
            param.requires_grad = False
        if args.arch in ['efficientb0', 'mobilenetv3'] and (name not in ['classifier.weight','classifier.bias']):
            param.requires_grad = False
    # init the fc layer
