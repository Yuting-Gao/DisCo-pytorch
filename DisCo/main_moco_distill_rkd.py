...
def main_worker(gpu, ngpus_per_node, args):
    ...
    #triplet_criterion = L2Triplet(sampler=opts.triplet_sample(), margin=0.2)
    # at_criterion = AttentionTransfer()
    criterion_dist = RkdDistance().cuda(args.gpu)
    criterion_angle = RKdAngle().cuda(args.gpu)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, criterion_mse, criterion_dist, criterion_angle, optimizer, epoch, args)
...
def train(train_loader, model, criterion, criterion_mse, criterion_dist, criterion_angle, optimizer, epoch, args):
    ...
    dist_losses = AverageMeter('Dist Loss', ':.4e')
    angle_losses = AverageMeter('Angle Loss', ':.4e')
    ...
    if args.use_mse:
        progress = ProgressMeter(len(train_loader),
            [batch_time, data_time, losses, ce_losses, dist_losses, angle_losses, mse_losses, mse_losses_qk, top1, top5],
                                 prefix="Epoch: [{}]".format(epoch))
    else:
        progress = ProgressMeter(len(train_loader),
            [batch_time, data_time, losses, ce_losses, dist_losses, angle_losses,
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        # Fake-classes
        #e = torch.cat([student_q, student_qk], dim=0)
        #t_e = torch.cat([teacher_q, teacehr_qk], dim=0)

        #e = torch.cat([student_q, student_qk], dim=0)
        #t_e = torch.cat([teacher_q, teacehr_qk], dim=0)
        e = student_q
        t_e = teacher_q
        #assert e.size() == (256, 128)
        #assert t_e.size() == (256, 128)

        dist_loss = criterion_dist(e, t_e)
        angle_loss = criterion_angle(e, t_e)
        ce_loss = criterion(output, target)

        # triplet_loss = triplet_criterion(e, labels)
        # dark_loss = dark_criterion(e, t_e)
        mse_loss = criterion_mse(teacher_q, student_q)
        mse_loss_qk = criterion_mse(teacher_qk, student_qk)

        if args.use_mse:
            loss = ce_loss  + dist_loss + angle_loss + mse_loss + mse_loss_qk
        else:
            loss = ce_loss  + dist_loss + angle_loss + 0.0 * mse_loss + 0.0 * mse_loss_qk
        ...
        dist_losses.update(dist_loss.item(), images[0].size(0))
        angle_losses.update(angle_loss.item(), images[0].size(0))
