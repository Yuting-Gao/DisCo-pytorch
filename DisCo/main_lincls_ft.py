...
from kd_ce_loss import loss_fn_kd
...
parser.add_argument('--teacher-arch', default='resnet50', type=str,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.teacher:
        if os.path.isfile(args.teacher):
            ...
            print("=> loaded checkpoint '{}' (epoch {})"
                  .format(args.teacher, checkpoint['epoch']))
        else:
    ...
    # load from pre-trained for student, before DistributedDataParallel constructor
...
def train(train_loader, model, teacher_model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        output = model(images)
        loss = criterion(output, target, teacher_output, args)
...
def validate(val_loader, model, teacher_model, criterion, args):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
