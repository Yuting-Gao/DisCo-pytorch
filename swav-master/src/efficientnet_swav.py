...
from .data_config import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD, IMAGENET_INCEPTION_MEAN, IMAGENET_INCEPTION_STD
...
def _cfg(url='', **kwargs):
    return {
        'url': url, 'num_classes': 128, 'input_size': (3, 224, 224), 'pool_size': (7, 7),
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, block_args, num_classes=128, num_features=1280,
                 in_chans=3, stem_size=32,
                 channel_multiplier=1.0, channel_divisor=8, channel_min=None,
                 output_stride=32, pad_type='', fix_stem=False,
                 act_layer=nn.ReLU, drop_rate=0., drop_path_rate=0.,
                 se_kwargs=None, norm_layer=nn.BatchNorm2d,
                 norm_kwargs=None, global_pool='avg',
                 output_dim=0, hidden_mlp=0, nmb_prototypes=0,
                 normalize=False,
        ...
        if output_dim == 0:
            ...
            self.projection_head = nn.Linear(1280,
                                             output_dim)
        else:
            self.projection_head = nn.Sequential(
                nn.Linear(1280, hidden_mlp),
    ...
    def forward(self, inputs):
        ...
        for end_idx in idx_crops:
            _out = self.forward_features(torch.cat(
...
class MultiPrototypes(nn.Module):
    def __init__(self, output_dim, nmb_prototypes):
        ...
        for i, k in enumerate(nmb_prototypes):
            self.add_module("prototypes" + str(i), nn.Linear(output_dim,
                                                             k, bias=False))
