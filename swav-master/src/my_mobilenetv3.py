...
class MobileNetV3(nn.Module):
    ...
    def __init__(self, block_args, num_classes=1000, in_chans=3, stem_size=16,
                 num_features=1280, head_bias=True,
                 channel_multiplier=1.0, pad_type='', act_layer=nn.ReLU,
        ...
        self.conv_head = create_conv2d(num_pooled_chs, self.num_features, 1,
                                       padding=pad_type, bias=head_bias)
        self.act2 = act_layer(inplace=True)
        # self.classifier = Linear(self.num_features, num_classes) if
        # num_classes > 0 else nn.Identity()
    ...
    def as_sequential(self):
        ...
        layers.extend([nn.Flatten(), nn.Dropout(self.drop_rate),
                       self.classifier])
